...
# Seurat's parallelization needs to be increased to prevent crashing
options(future.globals.maxSize = 2 * 1024^3)  # 4 GB
...
# Triana et al 2021 
# https://doi.org/10.6084/m9.figshare.13397987.v4
```{r}
# Download data from figshare if not already downloaded
# options(timeout = 3600)  # make sure download doesn't get timeout killed
# download.file(
#    url = "https://ndownloader.figshare.com/files/41038073",
#    destfile = "/home/nliu/scMultiOmics-normalization/data/41038073.rds",
#    mode = "wb")

triana_2021 <- readRDS(file = '/home/nliu/scMultiOmics-normalization/data/41038073.rds')
...
bm_data2 <- CreateSeuratObject(counts = bm_data@assays$RNA@layers$counts[rownames(bm_data@assays$RNA) %in% common_genes,])
...
# Seurat's default merge doesnt store one matrix but two counts matrices so we'll merge it manually 
```{r}
...
merged_data$log2_libsize <- log2(colSums(merged_counts))
merged_counts_assay <- SeuratObject::CreateAssay5Object(counts = merged_counts, key = 'rna_')
merged_data[['RNA']] <- merged_counts_assay
...
```{r}
merged_data$log2_rna_libsize <- log2(colSums(merged_data@assays$RNA@layers$counts))
merged_data$log2_adt_libsize <- log2(colSums(merged_data@assays$ADT@layers$counts))
...
rna_metrics <- BenchmarkMetrics(
  ...
  Raw_data = merged_data@assays$RNA@layers$counts, 
  Metadata = merged_data@meta.data)
rownames(rna_metrics@Raw_data) <- dimnames(merged_data@assays$RNA)[[1]]
colnames(rna_metrics@Raw_data) <- dimnames(merged_data@assays$RNA)[[2]]
...
adt_metrics <- BenchmarkMetrics(
  ...
  Raw_data = merged_data@assays$ADT@layers$counts, 
  Metadata = merged_data@meta.data)
rownames(adt_metrics@Raw_data) <- dimnames(merged_data@assays$ADT)[[1]]
colnames(adt_metrics@Raw_data) <- dimnames(merged_data@assays$ADT)[[2]]
...
test2 <- SingleCellExperiment::SingleCellExperiment(assays = list(RNA = rna_metrics@Raw_data),
                             colData = merged_data@meta.data)
# Write H5
anndataR::write_h5ad(test, path = "/home/nliu/scMultiOmics-normalization/data/ADT_merged.h5ad")
anndataR::write_h5ad(test, path = "/home/nliu/scMultiOmics-normalization/data/RNA_merged.h5ad")
rm(test)
rm(test2)
...
qs2::qs_save(workspace, "/home/nliu/scMultiOmics-normalization/data/panel2_backup.qs2")
rm(workspace)
gc()
...
workspace <- qs2::qs_read("/home/nliu/scMultiOmics-normalization/data/panel2_backup.qs2")
