...
```{r}
# svd the matrix
# split non-random error and e
# exp non-random error over e
# round
...
#' Back-transforms normalized logcounts into counts
...
test_smear <- smearing_transformation(x=test_fastruv, l = 68, pcs = 69, center = F)
...
plot((as.matrix(exp(assay(test_nb,'logPAC'))-1) %>% t())[,3], test_smear$adjusted[,3])
plot((as.matrix(exp(assay(test_nb,'logPAC'))-1) %>% t())[,3], exp(test_fastruv[,3]))
```
