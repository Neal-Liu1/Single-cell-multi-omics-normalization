...
liger_normalized <- rliger::normalize(bm_data@assays$RNA@layers$counts, log = F)
liger_normalized <- round(liger_normalized, digits = 2) %>% as.sparse()
rownames(liger_normalized) <- rownames(bm_data@assays$RNA)
...
plot_UMAP(liger_umap, run_umap = F, bm_data$celltype.l2)
...
p1 <- DimPlot(merged_data, reduction = 'rna_umap', group.by = 'study', label = T, repel = T) +
  ...
  xlab('umap1')+
  ylab('umap2')+
  theme(axis.line = element_line(colour = "grey75", linewidth=0.9),
          panel.border = element_rect(colour = "grey96", fill=NA, size=0.7),
          panel.grid.major = element_line(color = "grey96"),
          aspect.ratio = 1/1.05) +
  NoLegend()
...
rownames(merged_data@assays$ADT) <- common_adts
VariableFeatures(merged_data@assays$ADT) <- rownames(merged_data[['ADT']])

merged_data <- NormalizeData(merged_data, normalization.method = 'CLR', margin = 2)
