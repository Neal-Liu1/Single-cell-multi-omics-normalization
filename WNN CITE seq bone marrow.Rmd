...
```{r}
plot(rna_ruviii$W[,3], bm_data$log2_libsize)

plot(rna_ruviii$W[,3], col= as.factor(bm_data$celltype.l2))
...
# do pca & umap
rna_ruv_all_pca_all_genes <- run_PCA(rna_ruviii[hvgs], pcs = 50)
rna_ruv_all_umap_all_genes <- umap(rna_ruv_k6_pca$u[,1:30])

```

```{r}
plot_UMAP(rna_ruv_all_umap_all_genes$layout, bm_data$log2_libsize, run_umap = F, 
...
```
```{r}
Seurat::DimPlot(bm_data, reduction = 'rna.umap', group.by = 'celltype.l2', label = T, repel = T, pt.size =2.5)

```

# Test if libsize associates with biology
```{r}
summary(aov(bm_data$log2_libsize ~ bm_data$celltype.l2))

```

```{r, fig.height= 8}
plot_ridgeplot(bm_data$log2_libsize, bm_data$celltype.l2, aspect_ratio = 1/0.6)
...
p1 <- plot_linear_correlation(list(a,b, rna_ruv_k6_pca), c('rna_raw_counts', 'SCT', 'ruv_k6'), bm_data$log2_libsize, 'libsize', is.pca.obj = T)
p2 <- plot_vector_correlation(list(a,b, rna_ruv_k6_pca), c('rna_raw_counts', 'SCT', 'ruv_k6'), bm_data$donor, 'donor', is.pca.obj = T)
p3 <- plot_vector_correlation(list(a,b, rna_ruv_k6_pca), c('rna_raw_counts', 'SCT', 'ruv_k6'), bm_data$celltype.l2, 'celltype', is.pca.obj = T)
...
panc8.anchors <- Seurat::FindIntegrationAnchors(
  object.list = panc8.list, 
  anchor.features = features, 
  reduction = "rpca"
  )
panc8.combined <- Seurat::IntegrateData(
  anchorset = panc8.anchors
  )
```
...
rna_metrics@Silhouette$ruvIII_k6 <- compute_silhouette(rna_ruv_k6_umap$layout, 
                                                       bm_data$celltype.l2, 
                                                       result_format = 'per_cluster')
