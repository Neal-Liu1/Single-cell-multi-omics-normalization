...
all_silhouette_scores <- parallel::mclapply(list((adt_data),
                                                 (bm_data@assays$ADT@layers$data),
                                                 t(adt_dsb),
                                                 adt_adtnorm),
                                            function(x){compute_silhouette(matrix = x, label_vector = bm_data$celltype.l2, run_dim_reduction = 'umap',
                                                            result_format = 'per_cluster')})

all_silhouette_scores <- do.call(rbind, all_silhouette_scores)
all_silhouette_scores$algorithm <- rep(c('ADT_raw_counts', 'CLR', 'DSB', 'ADTnorm'), each = 27)
...
```{r}
plot_violin(all_silhouette_scores$silhouette_score, all_silhouette_scores$algorithm, names = c('Silhouette scores per cell type', 'Method')) +
  theme(axis.text.x = element_text(size = 10,angle = 45,hjust = 1))
...
plot_ridgeplot(tidyr::pivot_wider(all_silhouette_scores, 
                                   ...
                                   values_from = where(is.numeric))[,6:9], 
                label_vector = c('ADT_raw_counts', 'CLR', 'DSB', 'ADTnorm'),
...
MALT_tumour_10k <- Seurat::Read10X_h5(file = '/vast/scratch/users/liu.ne/malt_10k_protein_v3_raw_feature_bc_matrix.h5')
...
```{r}
plot_vector_correlation(list(adt_data, bm_data@assays$ADT$data, t(adt_dsb), adt_adtnorm), c('adt raw counts', 'CLR', 'DSB', 'ADTnorm'), 
...
```{r}
plot_linear_correlation(list(adt_data, bm_data@assays$ADT$data, t(adt_dsb), adt_adtnorm), c('adt raw counts', 'CLR', 'DSB', 'ADTnorm'), 
