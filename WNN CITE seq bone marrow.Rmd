...
```{r}
plot(rna_ruviii$W[,2], bm_data$log2_libsize)
...
# do pca & umap
rna_ruv_pca <- run_PCA(rna_ruviii[seurat_hvgs], pcs = 50)
rna_ruv_umap <- umap(rna_ruv_pca$u[,1:30])
...
```{r}
Seurat::DimPlot(bm_data1, reduction = 'rna.umap', group.by = 'celltype.l2', label = T, repel = T, label.size = 2.5) +
    theme_minimal() +
  labs(title = 'Cell types') +
  theme(axis.line = element_line(colour = "grey83", linewidth=1.1),
          panel.border = element_rect(colour = "grey90", fill=NA, size=0.7),
...
```{r}
triana_celltype_mappings <- lapply(triana_2021$ct, function(x){
  if(x %in% c()){}
})

# "B cell" = c("Pre-B cells", "Pro-B cells", "Plasma cells", "Class switched memory B cells",)
# "Mono/DC" = c("Conventional dendritic cell 1", "Conventional dendritic cell 2", "Plasmacytoid dendritic cells", "Classical Monocytes", 
# "Non-classical monocytes",)
# "NK" = c("CD56brightCD16- NK cells", "CD56dimCD16+ NK cells",)
# "Progenitor cells" = c("Late erythroid progenitor", "Late promyelocytes", "Megakaryocyte progenitors", "Early promyelocytes" , 
# "Lymphomyeloid prog", "Early erythroid progenitor", "Myelocytes", "Eosinophil-basophil-mast cell progenitors", "Plasmacytoid dendritic cell progenitors", 
# "HSCs & MPPs", "Erythro-myeloid progenitors", "NK cell progenitors", )
# "T cell" = c("CD4+ memory T cells", "CD8+CD103+ tissue resident memory T cells", )
