...
test <- Sparse_RUV_III(Y = log2(bm_data@assays$RNA@layers$counts+1),
                       Yrep = prpc,
                       M = ruv::replicate.matrix(prpc),
...
bm_data1 <- split(bm_data[['RNA']], bm_data$donor)

bm_data1 <- NormalizeData(bm_data1) %>% FindVariableFeatures() %>% ScaleData()
bm_data1 <- RunPCA(bm_data1)

bm_data1 <- FindNeighbors(bm_data1, dims = 1:30, reduction = "pca")
bm_data1 <- FindClusters(bm_data1, resolution = 2, cluster.name = "unintegrated_clusters")

View(Seurat::IntegrateLayers(bm_data1, 
                             method= RPCAIntegration, 
                             orig.reduction = 'pca',
                             new.reduction = "integrated_rpca"))

View(Seurat::IntegrateLayers(bm_data, 
