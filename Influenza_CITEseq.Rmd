...
test_seurat <- Seurat::FindClusters(
  ...
  cluster.name = 'harmony-wnn', resolution = 1.1)

adjustedRandIndex(test_seurat$`harmony-wnn`, test_seurat$K3)
```
...
```{r}
plot_UMAP(test@UMAPs$Seurat_CCA, test@Metadata$batch, run_umap = F, alpha = 0.35)
...
Sparse_RUV_III(Y= t(test@Raw_data),Yrep = t(test@Raw_data),M = ruv::replicate.matrix(test@Metadata$K3), k = 2, )
