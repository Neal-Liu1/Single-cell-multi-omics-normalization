...
p1 <- plot_UMAP(merged_adt_ruv_umap$layout, run_umap = F, metadata_vector = merged_data$celltype.l2_with_transfer, 
          title = 'RUV K5 (scaled) vs celltype ', alpha = 0.2)
...
```{r}
a<- list(Raw_counts = adt_pca_unscaled$u,
                         CLR = run_PCA((merged_data@assays$ADT@layers$data), pcs = 11)$u,
                         DSB = merged_adt_dsb_pca_unscaled$u,
...
merged_adt_LISI_scores <- parallel::mclapply(a, 
                         function(x){compute_lisi(x, 
                            meta_data = merged_data@meta.data, 
                            label_colnames = c('study','celltype.l2_with_transfer'))}) 

plot_violin(c(merged_adt_LISI_scores[[1]]$study, 
              merged_adt_LISI_scores[[2]]$study,
              merged_adt_LISI_scores[[3]]$study,
              merged_adt_LISI_scores[[4]]$study,
              merged_adt_LISI_scores[[5]]$study,
              merged_adt_LISI_scores[[6]]$study),
            rep(names(a), each = nrow(adt_pca_unscaled$u)), names = c('study'))
...
```{r}
plot_violin((c(merged_adt_LISI_scores[[1]]$celltype.l2_with_transfer, 
              merged_adt_LISI_scores[[2]]$celltype.l2_with_transfer,
              merged_adt_LISI_scores[[3]]$celltype.l2_with_transfer,
              merged_adt_LISI_scores[[4]]$celltype.l2_with_transfer,
              merged_adt_LISI_scores[[5]]$celltype.l2_with_transfer,
              merged_adt_LISI_scores[[6]]$celltype.l2_with_transfer)),
            rep(names(a), each = nrow(adt_pca_unscaled$u)), names= 'celltypes')
