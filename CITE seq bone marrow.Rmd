...
sample <- sample(nrow(merged_adt_metrics@PCs$Raw_counts), 1000)
test <- new('BenchmarkMetrics', 
            Algorithm = merged_adt_metrics@Algorithm,
            Metadata = merged_data@meta.data[sample,])
test@PCs <- lapply(merged_adt_metrics@PCs, function(x){x[sample,]})
test@Raw_data <- merged_adt_metrics@Raw_data[,sample]
...
params <- list()
test <- NormalizeADT(test, params = params)
...
setGeneric('ComputeHVG_Conservation', function(raw_hvgs, adj_matrix, flavour = 'vst')
  {standardGeneric('ComputeHVG_Conservation')})

setMethod('ComputeHVG_Conservation', 
          signature = c(raw_hvgs = 'logical',
                        adj_matrix = 'matrix'),
          function(raw_hvgs, adj_matrix, flavour){
            
            normalized_hvgs <- Seurat::FindVariableFeatures(
              adj_matrix,selection.method = flavour
              )[['variable']]
            
            return(mean(raw_hvgs == normalized_hvgs))
...
ComputeHVG_Conservation(Seurat::FindVariableFeatures(merged_adt_metrics@Raw_data, selection.method = 'vst')[['variable']],
                        t(merged_adtnorm))
