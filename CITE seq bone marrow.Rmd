...
# Compute & plot Multiple LISI functions/methods
```{r}
...
setMethod('ComputeMultipleLISI', 
          ...
          function(obj, reductions , variables, metadata, metrics_obj){
            ...
            if(length(obj@Reductions) == 0)
              {stop("Your Metrics obj doesn't have any reductions. Please add reductions first.")}
            
            reduction_matrix_list <- obj@Reductions
...
merged_adt_metrics@Reductions <- list(Raw_counts = adt_pca_unscaled$u,
                                      CLR = run_PCA((merged_data@assays$ADT@layers$data), pcs = 11)$u,
                                      DSB = merged_adt_dsb_pca_unscaled$u,
...
ggplot(data.frame(names = names(merged_ARIs), ARI = unlist(merged_ARIs)), aes(x= names , y = ARI, fill = names))+
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(title = "Bar Plot of ARIs",
       x = NULL,
       y = "ARI scores")+
  theme(panel.border=element_rect(colour = "grey85", fill=NA, size=0.8),
          aspect.ratio = 1,
          axis.line = element_line(colour = "grey70", linewidth = 1),
          axis.text.x = element_text(size = 10,angle = 45,hjust = 1),
          panel.grid.major = element_line(color = "grey96"),
        legend.position = 'none')
