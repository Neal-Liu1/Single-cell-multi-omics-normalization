...
merged_adt_metrics@PCs <- list(Raw_counts = adt_pca_unscaled$u,
...
adt_silhouettes <- parallel::mclapply(merged_adt_metrics@PCs,
                          function(x){compute_silhouette(matrix = x,
...
```{r}
PlotMultipleSilhouette(merged_adt_metrics)

```

```{r}

plot_linear_correlation(list(Raw_counts = adt_pca_unscaled,
                         CLR = run_PCA((merged_data@assays$ADT@layers$data), pcs = 11),
                         DSB = merged_adt_dsb_pca_unscaled,
                         Harmony = list(u=adt_harmony_unscaled),
                         RUVIII = merged_adt_ruv_pca_unscaled,
                         ADTnorm = merged_adtnorm_pca_unscaled),
                        dataset_names = c('Raw counts', 'CLR', 'DSB','Harmony', 'RUVIII', 'ADTnorm'),
                        merged_data$log2_adt_libsize,
                        'log2 adt libsize')
...
plot_vector_correlation(list(Raw_counts = adt_pca_unscaled,
                         CLR = run_PCA((merged_data@assays$ADT@layers$data), pcs = 11),
                         DSB = merged_adt_dsb_pca_unscaled,
                         Harmony = list(u=adt_harmony_unscaled),
                         RUVIII = merged_adt_ruv_pca_unscaled,
                         ADTnorm = merged_adtnorm_pca_unscaled),
                        dataset_names = c('Raw counts', 'CLR', 'DSB', 'Harmony', 'RUVIII', 'ADTnorm'),
                        merged_data$celltype.l2_with_transfer,
                        'celltypes',
                        num_pcs = 10)
...
setGeneric('ComputeARI',
           function(obj, labels, hclust_method = 'complete', distance_type  = 'euclidean'){standardGeneric('ComputeARI')})

setMethod('ComputeARI', 
...
```{r}
PlotCorrelations(merged_adt_metrics, variable = 'study', num_pcs = 10)

```

```{r}
PlotCorrelations(merged_adt_metrics, variable = 'log2_adt_libsize', num_pcs = 10)
