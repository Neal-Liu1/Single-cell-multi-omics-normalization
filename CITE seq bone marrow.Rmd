...
p1 <- plot_UMAP(merged_adt_ruv_umap_k5$layout, run_umap = F, metadata_vector = merged_data$celltype.l2_with_transfer, 
          title = 'RUV K2 (scaled) vs celltype ', alpha = 0.2)
...
merged_adt_metrics@Silhouette <- adt_silhouettes

```

```{r}
PlotMultipleSilhouette(merged_adt_metrics, aspect_ratio = 1.3, title = 'Silhouette score per celltype for 2 study adts')
...
```{r}
p1 <- PlotMultipleLISI(merged_adt_metrics, 'study', aspect_ratio = 1.1, title = 'Study LISI', levels = adt_levels)
p2 <- PlotMultipleLISI(merged_adt_metrics, 'celltype.l2_with_transfer', aspect_ratio = 1.1, 
                 title = 'Celltype LISI', levels = adt_levels)
gridExtra::grid.arrange(p1,p2, ncol =2)
...
setMethod('ComputeARIs', 
          signature = c(obj = 'BenchmarkMetrics', labels = 'character'),
          function(obj, labels, hclust_method, distance_type){
            label_name <- labels
            labels <- obj@Metadata[[labels]]
            
            clusters <- lapply(obj@PCs, function(x){
              cutree(hclust(d= dist(x, method = distance_type), 
                             method = hclust_method),
                      k= length(unique(labels)))
              })
            
            ARIs <- lapply(1:length(clusters), function(x){
              mclust::adjustedRandIndex(clusters[[x]], labels)
            })
    ...
    num_cross_validation){
            ...
            for (pc_name in names(obj@PCs)) {
              ...
              for (i in 1:num_cross_validation) {
                ...
                clusters <- cutree(hclust(d = dist(sampled_pc, method = distance_type), 
                                          method = hclust_method),
...
# The overall normalization method
```{r}
...
setMethod(
  ...
  function(obj, params, batch_variable, num_pcs)
    ...
    obj@RunningTime[['DSB']] <- Sys.time() - start
    ...
    obj@Adj_data[['CLR']] <- NormalizeData(
      ...
      margin = 2)
    obj@RunningTime[['CLR']] <- Sys.time() - start
    ...
    obj@RunningTime[['Harmony']] <- Sys.time() - start
    ...
    obj@RunningTime[['ADTnorm']] <- Sys.time() - start
...
test <- ComputeARIs(test, 'celltype.l2_with_transfer', sample_fraction = 0.05, num_cross_validation = 40)
test <- ComputeARIs(test, 'study', sample_fraction = 0.05, num_cross_validation = 40)
...
setGeneric(
  'NormalizeRNA',
  function(obj, params, batch_variable = 'batch', num_pcs = 30)
...
setMethod(
  ...
  function(obj, params, batch_variable, num_pcs)
    ...
    {stop("The batch variable name you entered doesn't exist in the metadata")}
    
    # Seurat log, totalVI, Seurat rPCA, Seurat CCA, SCT, RUVIII
    
    message('Computing PCA for raw data')
    ...
    message('Starting Seurat LogNormalize')
    start <- Sys.time()
    obj@Adj_data[['Seurat_LogNormalize']] <- Seurat::NormalizeData(obj@Raw_data)
    obj@RunningTime[['Seurat_LogNormalize']] <- Sys.time() - start
    
    message('Starting Seurat SCTransform')
    start <- Sys.time()
    obj@Adj_data[['SCTransform']] <- Seurat::SCTransform(obj@Raw_data, 
                                                         cell.attr = obj@Metadata,
                                                         return.only.var.genes = F)$y
    obj@RunningTime[['SCTransform']] <- Sys.time() - start
    
    message('Starting fastMNN')
    start <- Sys.time()
    obj@PCs[['fastMNN']] <- batchelor::fastMNN(obj@Raw_data,
                                 batch = obj@Metadata[[batch_variable]],
                                 subset.row = HVGs,
                                 d = num_pcs)@int_colData$reducedDims$corrected
    obj@RunningTime[['fastMNN']] <- Sys.time() - start
    
    message('FastMNN finished. Starting Harmony')
    start <- Sys.time()
    obj@PCs[['Harmony']] <- harmony::RunHarmony(
    ...
    "
    bm_list <- Seurat::SplitObject(obj, split.by = )
    bm_list <- lapply(X = bm_list, FUN = function(x) {
    x <- Seurat::NormalizeData(x)
    x <- Seurat::FindVariableFeatures(
      x, 
...
```{r}
sample2 <- sample(43000, 8000)
test2 <- new('BenchmarkMetrics',
             Raw_data = as.matrix(merged_counts[1:3000, sample2]),
             Metadata = merged_data@meta.data[sample2,])
test2@Algorithm <- c('Raw_data', 'Seurat_LogNormalize', 'SCTransform', 
                     'Seurat_rPCA', 'Seurat_CCA', 'Harmony', 
                     'fastMNN', 'totalVI', 'RUVIII')

rownames(test2@Raw_data) <- rownames(merged_data@assays$RNA)[1:3000]
...
# params = list(SCT_cell.attr = test2@Metadata$celltype.l2_with_transfer)

test2 <- NormalizeRNA(test2, 
...
```{r}
test2 <- ComputeMultipleLISI(test2, variables = c('study', 'celltype.l2_with_transfer'))
```
...
```{r}
PlotHVG_Conservation(test2, batch_variable = 'study', flavour = 'vst')
