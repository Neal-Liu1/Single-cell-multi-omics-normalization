...
i <- as.integer(args[1])
message("Starting benchmarking task: row ", i)
...
message("Loading simulation: ", simulation, ", seed: ", seed, ", cluster_res: ", cluster_res)

ground_truth_path <- paste0(
  "/vast/scratch/users/liu.ne/transformGamPoi_simulation_data/",
  simulation, "_seed_", seed, ".rds"
)
...
message("Running HVG selection using VST")
...
message("Finding neighbors and clusters at resolution: ", cluster_res)
...
message("Creating PRPC and finding NCGs")
...
for (k in k_values) {
  message("Applying fastRUVIII with k = ", k)
  ...
  for (pc in pcs) {
    for (nn in knns) {
      message("Evaluating with PC = ", pc, ", NN = ", nn)
      ...
      message("Saving results to ", out_path)
