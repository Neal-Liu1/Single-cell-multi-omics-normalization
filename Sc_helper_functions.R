...
setGeneric("ComputeARIs",
           function(obj,
                    label,                    # metadata column name (character scalar)
                    neighbours = c(10,20,50),               # numeric vector of k values (e.g., c(10,20,50))
                    resolutions = c(0.5, 1, 1.5),              # numeric vector of γ (e.g., c(0.8,1.0,1.2))
                    seeds = c(1,2,3),                    # integer vector of RNG seeds (e.g., 1:3)
                    metric = "euclidean",     # passed to Seurat::FindNeighbors(annoy.metric)
                    ...)
...
setMethod("ComputeARIs",
          signature = c(obj = "BenchmarkMetrics"),
          function(obj, label, neighbours, resolutions, seeds, metric = "euclidean", ...) {
            require(tidyverse)
            require(igraph)
            results <- cluster_and_score(
              pc_list = obj@PCs,
              truth_labels = obj@Metadata[[label]],
              grid_k  = neighbours,
              grid_g  = resolutions,
              seeds   = seeds,
              metric = metric)
            obj@ARI[[label]] <- results
            return(obj)
          })
...
setMethod(
  "PlotCorrelations",
  signature = c(obj = "BenchmarkMetrics", variable = "character", reductions = NULL),
  function(obj,
           variable,
           reductions,
           num_pcs,
    ...
    # convenience: method names used in plot
    methods_in_plot <- names(reductions)
    
    # If a palette is provided, restrict it to those methods (avoids warnings)
    pal_use <- NULL
    if (!is.null(palette)) {
      pal_use <- palette[intersect(names(palette), methods_in_plot)]
    }
    
    if (class(var) %in% c("factor", "character")) {
      dummies <- fastDummies::dummy_cols(var)[, -1]
      
      cancor_scores <- sapply(reductions, function(m) {
        lapply(1:num_pcs, function(y) {
          cca <- stats::cancor(x = m[, 1:y, drop = FALSE], y = dummies)
          1 - prod(1 - cca$cor^2)
        })
      }) %>% unlist()
      ...
      # Lock legend order: palette order if provided, otherwise obj@Algorithm, otherwise observed
      if (!is.null(pal_use)) {
        pc_correlations$Datasets <- factor(pc_correlations$Datasets, levels = names(pal_use))
      } else if (!is.null(obj@Algorithm)) {
        pc_correlations$Datasets <- factor(pc_correlations$Datasets, levels = obj@Algorithm)
      } else {
        pc_correlations$Datasets <- factor(pc_correlations$Datasets)
      }
      
      p <- ggplot(pc_correlations, aes(x = PCs, y = score, colour = Datasets, group = Datasets)) +
        geom_line(linewidth = 0.5, alpha = 0.8) +
        geom_point(alpha = 0.8) +
        labs(x = "Principal Components", y = "Correlation", colour = "Method") +
        ylim(0, 1) +
        theme_minimal() +
        theme(
          axis.line = element_line(colour = "grey83", linewidth = 1.1),
          panel.border = element_rect(colour = "grey90", fill = NA, linewidth = 0.7),
          panel.grid.major = element_line(color = "grey96"),
          aspect.ratio = 1 / 1.2
        ) +
        ggtitle(title)
      ...
      if (!is.null(pal_use)) {
        p <- p + ggplot2::scale_colour_manual(values = pal_use, breaks = names(pal_use), drop = drop_unused)
      }
    ...
    if (class(var) %in% "numeric") {
      R_squared <- sapply(reductions, function(matrix, var, num_pcs) {
        sapply(1:num_pcs, function(y) {
          summary(lm(var ~ matrix[, 1:y]))$r.squared
        })
      ...
      p <- ggplot(pc_correlations, aes(x = PCs, y = score, colour = Datasets, group = Datasets)) +
        geom_line(linewidth = 0.5, alpha = 0.8) +
        geom_point(alpha = 0.8) +
        labs(x = "Principal Components", y = "R-squared", colour = "Method") +
        ylim(0, 1) +
        ggtitle(title) +
        theme_minimal() +
        theme(
    ...
    stop("Unsupported variable type for PlotCorrelations: ", class(var)[1])
  }
)
...
setMethod(
  "PlotMultipleCorrelations",
  signature = c(obj = "BenchmarkMetrics", variables = "character"),
  function(obj, 
           variables, 
           reductions, 
           num_pcs, 
    ...
    plots[[1]] <- plots[[1]] +
      ...
      theme(
        ...
        axis.title.y = element_text(angle = 90, vjust = 2.5, colour = "grey40")
      )
...
add_totalVI <- function(metrics_obj, singlecellexperiment, assay_name, pca = T, pcs = 30){
  metrics_obj@Adj_data[['totalVI']] <- log2(SummarizedExperiment::assay(
    singlecellexperiment, assay_name)+1e-5) %>%
    as('dgCMatrix')
...
snn_to_igraph <- function(snn) {
  adj <- if (inherits(snn, "dgCMatrix")) snn else SeuratObject::as.sparse(snn)
  adj <- Matrix::forceSymmetric(adj, uplo = "U"); diag(adj) <- 0; adj <- Matrix::drop0(adj)
  rn <- rownames(adj); if (is.null(rn)) rn <- as.character(seq_len(nrow(adj)))
  ed <- summary(adj); ed <- ed[ed$i < ed$j, , drop = FALSE]
  graph_from_data_frame(
    data.frame(from = rn[ed$i], to = rn[ed$j], weight = ed$x),
    directed = FALSE, vertices = data.frame(name = rn)
  )
}


cluster_and_score <- function(
    pc_list,
    truth_labels,
    grid_k  = c(10, 20, 50),
    grid_g  = c(0.8, 1.0, 1.2),
    seeds   = 1:3,
    metric = 'euclidean'){
  # Ensure labels are named and factorized
  # stopifnot(!is.null(names(truth_labels)))
  truth <- as.factor(truth_labels)
  
  # Iterate
  res <- list()
  
  for (method in names(pc_list)) {
    PCs <- pc_list[[method]]
    stopifnot(is.matrix(PCs) || is.data.frame(PCs))
    PCs <- as.matrix(PCs)
    # Align ordering to labels by rowname
    rn <- rownames(PCs)
    if (is.null(rn)) stop("Each PC matrix must have rownames (cell IDs).")
    # missing_in_truth <- setdiff(rn, names(truth_labels))
    # if (length(missing_in_truth))
    #   stop(paste0("Some cells in ", method, " lack truth labels: ",
    #               paste(head(missing_in_truth, 5), collapse = ", "), " ..."))
    # # Reorder truth to PCs
    # truth <- truth_labels[rn]
    
    for (k in grid_k) {
      message(paste0("Finding Neighbours for k= ", k))
      g <- Seurat::FindNeighbors(PCs, k.param = k, annoy.metric = metric)[['snn']]
      g  <- snn_to_igraph(g)
      for (gamma in grid_g) {
        for (s in seeds) {
          set.seed(s)
          # igraph::cluster_leiden supports resolution_parameter
          message(paste0("Running Leiden for res ", gamma, " seed ", s))
          cl <- igraph::cluster_leiden(
            g, 
            resolution_parameter = gamma, 
            objective_function = "modularity")
          pred <- factor(membership(cl)[V(g)$name]) %>% as.vector()
          
          # compute metrics
          message("Computing ARI")
          ari <- mclust::adjustedRandIndex(pred, truth)
          message("Computing AMI")
          ami <- aricode::AMI(as.integer(pred), as.integer(truth))  # AMI expects ints OK
          message(paste0("ARI: ", ari,"AMI: ", ami))
  ...
  ari_summary_df <- results_df %>%
    group_by(method, k, gamma) %>%
    summarise(ARI_med = median(ARI), ARI_min = min(ARI), ARI_max = max(ARI),
              .groups = "drop")
  
  ami_summary_df <- results_df %>%
    group_by(method, k, gamma) %>%
    summarise(AMI_med = median(AMI), AMI_min = min(AMI), AMI_max = max(AMI),
              .groups = "drop")
...
plot_ami_ari <- function(df, title, palette) {
  # lock consistent legend + colour mapping
  df$Method <- factor(df$Method, levels = names(palette))
  
  ggplot(df, aes(x = AMI_celltype, y = ARI_celltype)) +
    geom_point(aes(colour = Method), size = 2.5, alpha = 0.4) +
    
    # highlight supervised
    geom_point(
      data = subset(df, Method == "fastRUVIII_supervised"),
      colour = "black", size = 4.5, alpha = 0.9
    ) +
    geom_point(
      data = subset(df, Method == "fastRUVIII_supervised"),
      aes(colour = Method), size = 4, alpha = 0.6
    ) +
    
    # highlight unsupervised
    geom_point(
      data = subset(df, Method == "fastRUVIII_unsupervised"),
      colour = "black", size = 4.5, alpha = 0.9
    ) +
    geom_point(
      data = subset(df, Method == "fastRUVIII_unsupervised"),
    ...
    scale_method_colour(palette = palette) +
    theme_minimal(base_size = 14) +
    labs(y = "ARI", x = "AMI", title = title) +
    theme(
      aspect.ratio = 1,
      panel.border = element_rect(colour = "grey55", fill = NA, linewidth = 1)
    )
}


.apply_named_colour_scale <- function(palette, drop = TRUE) {
...
scale_method_colour <- function(palette, ...) {
  ggplot2::scale_colour_manual(
    values = palette, 
    breaks = names(palette), 
    drop = TRUE, ...)
...
scale_method_fill <- function(palette, ...) {
